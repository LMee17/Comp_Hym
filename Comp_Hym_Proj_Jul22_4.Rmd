...
```{r}
sig.ort <- ort.48.res.sig$OrthoGroup
ann.slim[ann.slim$Orthogroup %in% sig.ort,]
```

OG0003913: tyrosine hydrolase

Background gene, sig (FDR < 0.1) in Gram Pos treatment
...
Canonical immune signaling orthogroup sig (FDR < 0.01) in Gram Neg treatment
...
##OrthoLevel DE: All Bees (Anthophila)
...
Background orthogroup sig (FDR < 0.05) in Gram Pos treatment
...
Background orthogroup, sig (FDR < 0.1) in Gram Pos
...
Canonical immune signaling orthogroup sig (FDR < 0.1) in Gram Neg treatment
...
Background orthogroup sig (FDR < 0.1) in Gram Neg
...
Putative immune gene, sig (FDR < 0.1) in Gram Pos treatment
...
```{r}
ann.slim[ann.slim$Orthogroup == "OG0000399",]
```
Canonical immune recognition orthogroup sig (FDR < 0.01) in Gram Pos treatment
...
writeLines(capture.output(sessionInfo()),
           "SessionLogs/Proj_Pt4_SesInfo.txt")
sessionInfo()
```
